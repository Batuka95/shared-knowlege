...
eventsfile="/data/local/tmp/eventsld"
...
#echo "x ${x:2:2}${x:0:2}"
printf "\x${x:2:2}\x${x:0:2}" | busybox dd of=$eventsfile bs=1 seek=44 count=2 conv=notrunc > /dev/null 2>&1
printf "\x${y:2:2}\x${y:0:2}" |	busybox dd of=$eventsfile bs=1 seek=60 count=2 conv=notrunc > /dev/null 2>&1
#echo "y ${y:2:2}${y:0:2}"
#printf "\x${y:2:2}\x${y:0:2}" | busybox dd of=$eventsfile bs=1 seek=28 count=2 conv=notrunc
#printf "\x${y:2:2}\x${y:0:2}" | busybox dd of=$eventsfile bs=1 seek=92 count=2 conv=notrunc
...
#hexdump $eventsfile
cat $eventsfile > /dev/input/event3

echo "done"

#printf "\xFF\xFF" | busybox dd of=$eventsfile bs=1 seek=12 count=2 conv=notrunc